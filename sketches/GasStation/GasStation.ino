...
void readUsers()
{
	...
	if (!SD.mkdir("TESTDIR"))
		Serial.println("mkdir fails");
	if (!SD.exists("TESTDIR"))
		Serial.println("TESTDIR dont exists");
	if (!SD.exists("o_list.txt"))
		Serial.println("o_list.txt dont exists");

	userFile = SD.open("o_list.txt", FILE_READ);
	if (!userFile)
...
void readVehicles()
{
	File vehicleFile;
	LICZBA_POJAZDOW = 0;
	vehicleFile = SD.open("p_list.txt", FILE_READ);
	if (!vehicleFile)
